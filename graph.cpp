...
Graph* Graph::dijkstra(Graph* graph, int start) {
    ...
    for (Graph::Edge* v : adjacency_list) {
        v->data.second = inf;
        v->prev = NULL;
    }
...
void Graph::addEdge(int index, Edge* edge) {
    ...
    Graph::Edge* temp = adjacency_list[index]->next;
...
void Graph::replace(Edge* toReplace, Edge* toInsert) {
    pq.removeElement(toReplace);
    pq.push(toInsert);
}

void Graph::removeElement(Edge* element) {
    std::priority_queue <Edge*, std::vector<Edge*>, EdgeComparator> pq_helper;

    while (!(pq.empty())) {
        if (pq.top()->data.second != element->data.second) {
